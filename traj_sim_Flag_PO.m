function [Flag, Burn2] = traj_sim_Flag_PO(Stage_Sep_time,Pitch_Over_Angle)
% function [Flag, Burn2] = traj_sim_Flag_PO(Stage_Sep_time)
...
for ct = 1:InitVert
    V_2 = V_1 + (mfuel_dot_s1 * V_exh_1/M_1) - g_1*Time_step;
    Alt_2 = Alt_1 + V_1*Time_step;
    R_2 = R_1 + V_1*Time_step;
...
while PO_Target > 0
    V_2 = V_1 + (mfuel_dot_s1 * V_exh_1/M_1) - (g_1*Time_step*sin(Gamma_1));
    Alt_2 = Alt_1 + (V_1*Time_step*sin(Gamma_1));
    R_2 = R_1 + (V_1*Time_step*sin(Gamma_1));
    M_2 = M_1 - mfuel_dot_s1;
    g_2 = GM/(R_1^2);
%     g_2 = GM/(R_1^2);
...
%     if Gamma_2>deg2rad(40)
        Gamma_2 = Gamma_1 + ((V_1*Time_step)*cos(Gamma_1)/R_1) - ((1/V_1)*g_1*Time_step*cos(Gamma_1));
...
for ct = 1:Stage_Sep_time
    V_2 = V_1 - (g_1*Time_step*sin(Gamma_1));
    Alt_2 = Alt_1 + (V_1*Time_step*sin(Gamma_1));
    R_2 = R_1 + (V_1*Time_step*sin(Gamma_1));
    M_2 = M_1;
    g_2 = GM/(R_1^2);
...
while M_1 > 47.4
    V_2 = V_1 + (mfuel_dot_s2 * V_exh_2/M_1) - (g_1*Time_step*sin(Gamma_1));
    Alt_2 = Alt_1 + (V_1*Time_step*sin(Gamma_1));
    R_2 = R_1 + (V_1*Time_step*sin(Gamma_1));
...
%     end
    ...
    if V_2>3377 && V_2 <3397 && Alt_2 >333000 && Alt_2 <353000 && Gamma_2 <5 && Gamma_2 > -5 % Success
        Flag = 1;
        break
    else
        Flag = 0;
